function [] = myPCADenoising2_poisson(img, divide_fact)
rng(0);

im = double(imread(img));
im1 = poissrnd(im/divide_fact);
im1 = sqrt(im1);
sigma = 0.5;
...
imshow(mat2gray(im1));
title('Noisy Image, dividing factor = ' + string(divide_fact));

figure;
imshow(mat2gray(im2));
title('Denoised Image, dividing factor = ' + string(divide_fact));
